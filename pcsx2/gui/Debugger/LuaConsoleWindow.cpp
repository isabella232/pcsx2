...
void LuaConsoleWindow::OnLuaEdit(wxCommandEvent& event)
{
	...
	if (exists) {
		...
		ScopedPtr<wxFileType> luaFileType(wxTheMimeTypesManager->GetFileTypeFromExtension(L"lua"));
		if (luaFileType) {
			wxArrayString verbs;
		...
		if (command.IsEmpty()) {
			ScopedPtr<wxFileType> textFileType(wxTheMimeTypesManager->GetFileTypeFromMimeType(L"text/plain"));
			if (textFileType) {
